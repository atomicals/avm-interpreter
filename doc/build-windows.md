...
Below are some notes on how to build AVM Interpretor for Windows.

Please note that from AVMN v0.21.3 onwards, building for Win32 is no longer
officially supported (and build system capabilities related to this may
be removed).

The options known to work for building AVM Interpretor on Windows are:
...
- On Linux, using the [Mingw-w64](https://www.mingw-w64.org/downloads/) cross compiler
  ...
  AVM Interpretor Windows release binaries.
...
Cash Node natively under Windows. If you just want to run AVM Interpretor,
you can download binaries from the [AVM Interpretor website](https://atomicals.xyz/en/download.html).
If you wish to both compile and run AVM Interpretor on Windows, *under WSL*,
...
Note that for WSL 2 the AVM Interpretor source path MUST be somewhere in the default
mount file system, for example `/usr/src/avm`, AND not under `/mnt/d/`.
...
```bash
    ...
    mkdir build
    cd build
    cmake -GNinja .. -DCMAKE_TOOLCHAIN_FILE=../cmake/platforms/Win64.cmake -DENABLE_MAN=OFF -DBUILD_AVM_SEEDER=OFF # seeder not supported in Windows yet
    ninja
...
way. This will install to `c:\workspace\avm`, for example:

```bash
    cmake -GNinja .. -DCMAKE_TOOLCHAIN_FILE=../cmake/platforms/Win64.cmake -DENABLE_MAN=OFF -DBUILD_AVM_SEEDER=OFF -DCMAKE_INSTALL_PREFIX=/mnt/c/workspace/avm
    sudo ninja install
```
...
AVM Interpretor. To install the latest version:
...
Note that for WSL the AVM Interpretor source path MUST be somewhere in the default
mount file system, for example `/usr/src/avm`, AND not under `/mnt/d/`.
This means you cannot use a directory that is located directly on the host Windows
file system to perform the build.
...
### Building AVMN installer

To build a Windows installer for AVMN you need a newer version of the `nsis` package
...
`std::mutex`. It's not possible to build the AVM Interpretor code using the win32
version of the Mingw-w64 cross compilers (at least not without modifying headers
in the AVM Interpretor source code).
