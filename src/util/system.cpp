...
const char *const AVM_CONF_FILENAME = "avm.conf";
...
public:
    ...
    static inline bool UseDefaultSection(const ArgsManager &am,
        ...
        return (am.m_network_only_args.count(arg) == 0);
    }
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Atomicals";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Atomicals";
#else
    // Unix
    return pathRet / ".atomicals";
#endif
#endif
}
